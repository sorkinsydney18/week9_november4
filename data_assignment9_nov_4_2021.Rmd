...
disgust_diff <- difference_in_means(E_disgust ~ disgust_cond, data = study2_test)
anxiety_diff <- difference_in_means(E_anxiety ~ disgust_cond, data = study2_test)
anger_diff <- difference_in_means(E_angry ~ disgust_cond, data = study2_test)


conditions <- c('Low Disgust', 'High Disgust')

disgust_point <- c(1.984416, 	2.564738)
anxiety_point <- anxiety_diff$coefficients
anger_point <- anger_diff$coefficients

lowbounds_disgust <- disgust_diff$conf.low
upbounds_disgust <- disgust_diff$conf.high

lowbounds_anxiety <- anxiety_diff$conf.low
upbounds_anxiety <- anxiety_diff$conf.high

lowbounds_anger <- anger_diff$conf.low
upbounds_anger <- anger_diff$conf.high


disgust_treatment_effect_plot_data <- tibble(conditions,disgust_point,lowbounds_disgust,upbounds_disgust)

ggplot(disgust_treatment_effect_plot_data, 
       aes(x=factor(conditions, levels = conditions), 
           y=disgust_point)) + geom_point()


  xlab('Experimental Outcomes') + 
